...
function sendJoinPrompt(chatId) {
  bot.sendMessage(chatId, `⚠️ **Access Denied!**\n\nYou must join our official group first to use this bot.`, { reply_markup: { inline_keyboard: [[{ text: "📢 Join Group", url: GROUP_INVITE_LINK }], [{ text: "🔄 Check Again", callback_data: "check_join" }]] }, parse_mode: "Markdown" });
}
...
bot.onText(/\/start(?:\s+(.+))?/, async (msg, match) => {
  ...
  if (!db.users.includes(chatId)) {
      ...
      if (refId && Number(refId) !== chatId && !db.referred[chatId]) { db.referred[chatId] = Number(refId); addBalance(Number(refId), 10.00); bot.sendMessage(Number(refId), `🎉 **Congratulations!**\nA new user just joined using your link. 💰 **10.00 BDT** added!`, { parse_mode: "Markdown" }).catch(()=>{}); }
      saveDB();
  }
  bot.sendMessage(chatId, `Welcome! 👋\n\nPlease select an option below:`, { reply_markup: getReplyMenu(chatId, username) });
});

bot.onText(/\/admin/, async (msg) => {
  if (!isAdmin(msg.chat.id, msg.from.username)) return bot.sendMessage(msg.chat.id, "❌ Access Denied.");
...
bot.on('message', async (msg) => {
  ...
  if (text === "☎️ Get Number") { 
    ...
    try {
        ...
        bot.sendMessage(chatId, `📧 **Your Temp Mail:**\n\`${email}\`\n\n⏳ Auto-checking inbox...`, { parse_mode: "Markdown" });

        const interval = setInterval(async () => {
            try {
                const inbox = await (await fetch(`https://www.1secmail.com/api/v1/?action=getMessages&login=${login}&domain=${domain}`)).json();
                if (inbox.length > 0) {
                    const latest = inbox[0];
                    if (activeTempMails[chatId].lastId !== latest.id) {
                        activeTempMails[chatId].lastId = latest.id;
                        const msgData = await (await fetch(`https://www.1secmail.com/api/v1/?action=readMessage&login=${login}&domain=${domain}&id=${latest.id}`)).json();
        ...
        const timeout = setTimeout(() => {
            ...
            bot.sendMessage(chatId, "⚠️ Temp mail session expired (15m). Request a new one.", { parse_mode: "Markdown" });
        }, 15 * 60 * 1000);
      ...
      bot.sendMessage(chatId, "🔐 **Send your secret key:**\n(For example: `RTOX IVWV MK7A...`)", { parse_mode: "Markdown" });
  }
    ...
    if (isNaN(limit) || limit < 1 || limit > 20) bot.sendMessage(chatId, "❌ Invalid limit.");
    else { db.settings.maxNumbers = limit; saveDB(); bot.sendMessage(chatId, `✅ Updated!`); bot.sendMessage(chatId, "⚙️ **Admin Panel:**", { reply_markup: getAdminMenu(chatId) }); delete userStates[chatId]; }
  }
    ...
    bot.sendMessage(chatId, `✅ **Sent to ${successCount} users.**`); delete userStates[chatId];
  }
...
bot.on('callback_query', async (query) => {
  ...
  if (data === "check_join") {
    ...
    else return bot.answerCallbackQuery(query.id, { text: "❌ Join group first!", show_alert: true });
  }
  if (!await isUserMember(query.from.id)) return bot.answerCallbackQuery(query.id, { text: "❌ Join group first!", show_alert: true });
  
  const adminActs = ["admin_", "togglerng_", "refresh_manage", "deladmin_", "addnum_", "placeholder_stex", "placeholder_mk"];
  ...
  if (data === "close_menu") { bot.deleteMessage(chatId, messageId).catch(()=>{}); return bot.answerCallbackQuery(query.id); }
    ...
    bot.editMessageText("🛠 **Select Platform:**", { chat_id: chatId, message_id: messageId, reply_markup: adminPlatformMenu });
  ...
  else if (data === "withdraw_funds") { userStates[chatId] = "WAITING_FOR_BKASH"; bot.sendMessage(chatId, "💸 **Enter number:**"); bot.answerCallbackQuery(query.id); }
    ...
    else { const idx = parseInt(action), sel = tempAdminData[chatId].ranges[idx]; if (db.availableNumbers[sel.name]) { delete db.availableNumbers[sel.name]; saveDB(); bot.answerCallbackQuery(query.id, { text: `❌ Removed` }); } else { db.availableNumbers[sel.name] = []; let a = 0; sel.nums.forEach(num => { if (!inUseNumbers[num]) { db.availableNumbers[sel.name].push(num); a++; } }); saveDB(); bot.answerCallbackQuery(query.id, { text: `✅ Added (${a})` }); } }
    renderManageRangesMenu(chatId, messageId);
  }
...
mongoose.connect(MONGODB_URI).then(async () => {
  ...
  isDbLoaded = true; app.listen(PORT, () => console.log(`🚀 Node Server Active`));
