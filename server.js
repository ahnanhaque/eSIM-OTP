...
// ⚠️ YOUR BOT CONFIGURATION HERE (No need for .env)
const botToken = "8529122267:AAEjUc_8-EcNeHnwP1YPT6FX8wB51k35qKg"; 
const ADMIN_ID = 8278612952; // Replace with your numeric Telegram Admin ID
const GROUP_CHAT_ID = -1003852968469; // Replace with your numeric Group ID
...
function getAdminMenu(chatId) {
  let menu = [
    ...
    [{ text: "⚙️ Manage Ranges", callback_data: "admin_manage_ranges" }, { text: "➕ Add Number", callback_data: "admin_add_number" }],
...
bot.onText(/\/admin/, async (msg) => {
  ...
  if (!isAdmin(chatId, username)) return bot.sendMessage(chatId, "❌ You don't have admin rights!");
  bot.sendMessage(chatId, "⚙️ **Admin Panel:**\n\nHere you can manage iVAS numbers for the bot.", { reply_markup: getAdminMenu(chatId), parse_mode: "Markdown" });
});
...
bot.on('message', async (msg) => {
  ...
  // ⚡ Cookies sent directly to iva.js Memory
    ...
    const xsrf = xsrfMatch ? xsrfMatch[1].trim() : "";
    const session = sessionMatch ? sessionMatch[1].trim() : "";
    if (!session) { bot.sendMessage(chatId, "❌ **ivas_sms_session** cookie string এ পাওয়া যায়নি! আবার পাঠান।", { parse_mode: "Markdown" }); return; }
    db.cookies["XSRF-TOKEN"] = xsrf;
    db.cookies["ivas_sms_session"] = session;
    saveDB();
    iva.setCookies(xsrf, session);
    cachedToken = null;
    bot.sendMessage(chatId, "✅ **Cookies have been successfully updated directly to IVA Engine!**");
    ...
    let added = 0;
    numbers.forEach(num => { if (!db.availableNumbers[country].includes(num)) { db.availableNumbers[country].push(num); added++; } });
    saveDB();
...
bot.on('callback_query', async (query) => {
  ...
  if (data === "close_menu") { bot.deleteMessage(chatId, messageId).catch(()=>{}); return bot.answerCallbackQuery(query.id); }
  else if (data === "admin_update_cookies") { userStates[chatId] = "WAITING_FOR_COOKIES"; bot.sendMessage(chatId, "🍪 **Update iVAS Cookies:**\n\nপুরো cookie string পাঠান (যেখানে **ivas_sms_session** এবং **XSRF-TOKEN** দুটোই আছে):", { parse_mode: "Markdown" }); bot.answerCallbackQuery(query.id); }
    ...
    try {
      ...
      if (!token) {
        return bot.editMessageText(`❌ **Session Expired!**\n\nYour cookies are invalid or expired. Please click **"🍪 Update Cookies"** from the Admin Panel to submit new values.`, { chat_id: chatId, message_id: messageId, reply_markup: { inline_keyboard: [[{ text: "⬅️ Back", callback_data: "admin_panel" }]] }, parse_mode: "Markdown" });
      }
    ...
    for (let i = 0; i < countries.length; i += 2) {
      const row = [{ text: `${countryData[countries[i]].flag} ${countries[i]}`, callback_data: `addnum_${countries[i]}` }];
      if (countries[i+1]) row.push({ text: `${countryData[countries[i+1]].flag} ${countries[i+1]}`, callback_data: `addnum_${countries[i+1]}` });
      countryBtns.push(row);
    }
    countryBtns.push([{ text: "⬅️ Back to Admin", callback_data: "admin_panel" }]);
    bot.editMessageText("➕ **Add Numbers:**\n\nCountry select করুন:", { chat_id: chatId, message_id: messageId, reply_markup: { inline_keyboard: countryBtns }, parse_mode: "Markdown" });
    bot.answerCallbackQuery(query.id);
  }
  else if (data.startsWith("addnum_")) {
    const country = data.replace("addnum_", "");
    tempAdminData[chatId] = { addNumberCountry: country };
    userStates[chatId] = "WAITING_FOR_ADD_NUMBERS";
    const info = getCountryInfo(country);
    bot.sendMessage(chatId, `➕ **${info.flag} ${country}** তে নম্বর যোগ করুন:\n\nএকটি করে লাইনে নম্বর পাঠান:`, { parse_mode: "Markdown" });
    bot.answerCallbackQuery(query.id);
  }
...
// 💡 This shared function prevents Heavy and Fast checkers from skipping OTPs!
function processFoundOTP(number, time, message, range) {
  const uniqueId = `${number}_${time}`;
  
  if (lastProcessedOTPTime[uniqueId]) return; // Already processed, skip
  lastProcessedOTPTime[uniqueId] = true;      // Mark as processed
  ...
  // 1. Send to the public Telegram group
  ...
  // 2. If a user is waiting for this number, update their UI menu
  if (pendingRequests[number]) {
    ...
    addBalance(reqData.chatId, 0.50); // Add balance for successful OTP
...
// ⚡ Targeted Fast Checker: Calls iva.js functions and uses the shared processor
...
// 🌐 Heavy Background Checker: Calls iva.js getSMS and uses the shared processor
    ...
    if (token) {
      ...
      if (resData.aaData && resData.aaData.length > 0) {
        resData.aaData.forEach(row => {
          // row format: [time, range, number, sender, message]
          // 💡 Use shared function here
...
app.listen(PORT, () => {
  ...
  // ⚡ The fast checker hits only when a user is waiting (every 2 seconds)
  setInterval(fastPendingOTPCheck, 2000);
  
  // 🌐 The general background checker catches missed items (every 15 seconds)
