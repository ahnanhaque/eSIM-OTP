...
function getCountryInfo(countryName) {
  ...
  for (const key in countryData) {
    if (upperName.includes(key)) { flag = countryData[key].flag; cleanName = key.split(' ').map(w => w.charAt(0).toUpperCase() + w.slice(1).toLowerCase()).join(' '); break; }
  }
...
function buildAssignedMessageText(country, numsList, statuses) {
  ...
  let text = "";
  numsList.forEach((num, index) => {
    text += `☁️ **eSIM OTP** ☁️\n🌍 Country: ${info.flag} ${info.cleanName.toUpperCase()}\n📞 Number: \`${num}\`\n📩 SMS Status: ${statuses[num]}\n`;
    if (index < numsList.length - 1) text += `━━━━━━━━━━━━━━━━━━━━━━━\n\n`;
  });
  return text;
}

function clearPendingForChat(chatId) {
  for (let num in pendingRequests) { if (pendingRequests[num].chatId === chatId) { delete inUseNumbers[num]; delete pendingRequests[num]; } }
}
...
function getAdminMenu(chatId) {
  let menu = [
    [{ text: "📢 Broadcast Message", callback_data: "admin_broadcast" }, { text: "⚙️ Manage Ranges", callback_data: "admin_manage_ranges" }],
    [{ text: "📊 Check Stored Numbers", callback_data: "admin_check_db" }, { text: "🔢 Set Number Limit", callback_data: "admin_set_limit" }],
...
function renderManageRangesMenu(chatId, messageId) {
  ...
  rangesArray.forEach((r, index) => {
    ...
    rangeButtons.push([{ text: `${isAdded ? "✅" : "❌"} ${info.flag} ${r.name} (${r.nums.length})`, callback_data: `togglerng_${index}` }]);
  });
...
bot.on('message', async (msg) => {
  ...
  // 🟢 Proxy Configuration Listener
    ...
    if (parts.length === 4) {
        const host = parts[0].trim(); const port = parts[1].trim(); const user = parts[2].trim(); const pass = parts[3].trim();
        db.settings.proxy = { host, port, user, pass }; saveDB();
        if (typeof iva.setProxyConfig === 'function') iva.setProxyConfig(host, port, user, pass);
        bot.sendMessage(chatId, `✅ **Proxy Updated Successfully!**\n\n🌐 **Host:** ${host}\n🔌 **Port:** ${port}\n👤 **User:** ${user}`, { parse_mode: "Markdown" });
        bot.sendMessage(chatId, "🔑 **Select a panel to login:**", { reply_markup: loginPanelMenu, parse_mode: "Markdown" });
    } else {
        bot.sendMessage(chatId, "❌ Invalid format. Please enter exactly as: `Host, Port, Username, Password`\nExample: `brd.superproxy.io, 33335, user-123, pass_123`", { parse_mode: "Markdown" });
    }
  ...
  // 🟢 Login Input Listener
    ...
    const parts = text.split(',');
    if (parts.length === 2) {
        const email = parts[0].trim(); const password = parts[1].trim();
        bot.sendMessage(chatId, "⏳ Logging into IVAS via Bright Data Web Unlocker. Please wait...");
        iva.loginIVAS(email, password).then(res => {
            if (res.success) {
                bot.sendMessage(chatId, "✅ **Successfully logged in to IVAS!**\nCookies have been updated in the bot's memory automatically.", { parse_mode: "Markdown" });
                
                // Save cookies to MongoDB to survive restarts
                const currentCookies = iva.getCookies();
                db.cookies = db.cookies || {};
                db.cookies["XSRF-TOKEN"] = currentCookies.xsrf;
                db.cookies["ivas_sms_session"] = currentCookies.session;
                saveDB();
                cachedToken = null; 
            } else {
                bot.sendMessage(chatId, "❌ **Login failed:** " + res.error, { parse_mode: "Markdown" });
            }
        });
    } else {
        bot.sendMessage(chatId, "❌ Invalid format. Please enter exactly as: `email, password`", { parse_mode: "Markdown" });
    }
    delete userStates[chatId];
  }
    ...
    if (/^(01[3-9]\d{8})$/.test(text)) {
      ...
      if (currentBalance < 50) { bot.sendMessage(chatId, `⚠️ Insufficient balance.`); delete userStates[chatId]; return; }
    ...
    else bot.sendMessage(chatId, `⚠️ Already an admin!`);
...
bot.on('callback_query', async (query) => {
  ...
  if ((data.startsWith("admin_") || data.startsWith("togglerng_") || data.startsWith("refresh_") || data.startsWith("deladmin_")) && !isAdmin(chatId, username)) return bot.answerCallbackQuery(query.id, {text: "❌ Permission Denied!", show_alert: true});
  ...
  // 🟢 Admin Menu Buttons Handlers
  else if (data === "admin_login_panel") { bot.editMessageText("🔑 **Select a panel to login:**", { chat_id: chatId, message_id: messageId, reply_markup: loginPanelMenu, parse_mode: "Markdown" }); bot.answerCallbackQuery(query.id); }
  else if (data === "admin_set_proxy") { userStates[chatId] = "WAITING_FOR_PROXY"; bot.sendMessage(chatId, "🌐 **Enter new Proxy details:**\nFormat: `Host, Port, Username, Password`\nExample: `brd.superproxy.io, 33335, user-123, pass_123`", { parse_mode: "Markdown" }); bot.answerCallbackQuery(query.id); }
  else if (data === "admin_login_ivas") { userStates[chatId] = "WAITING_FOR_IVAS_CREDS"; bot.sendMessage(chatId, "🔐 **Enter your IVAS login details:**\nFormat: `email, password`\nExample: `admin@example.com, mypass123`", { parse_mode: "Markdown" }); bot.answerCallbackQuery(query.id); }
  else if (data === "placeholder_stex" || data === "placeholder_mk") { bot.answerCallbackQuery(query.id, { text: "🛠 This service is not integrated yet.", show_alert: true }); }
    ...
    ranges.forEach(range => {
      let info = getCountryInfo(range), displayName = `${info.flag} ${info.cleanName}`;
      if (baseCountryCount[info.cleanName] > 1) { currentV[info.cleanName] = (currentV[info.cleanName] || 0) + 1; displayName += ` V${currentV[info.cleanName]}`; }
      displayName += ` | 📦: ${db.availableNumbers[range].length}`;
      countryButtons.push([{ text: displayName, callback_data: `assign_${range}` }]);
    });
    countryButtons.push([{ text: "🔄 Restore Last Number", callback_data: "restore_last" }], [{ text: "❌ Close Menu", callback_data: "close_menu" }, { text: "⬅️ Back", callback_data: "menu_platform" }]);
  ...
  else if (data === "restore_last") {
    const last = db.lastAssigned[chatId];
    if (!last || !last.nums || last.nums.length === 0) return bot.answerCallbackQuery(query.id, { text: "⚠️ No record of previous numbers!", show_alert: true });
    clearPendingForChat(chatId); const limit = db.settings.maxNumbers || 4;
    const actionMenu = { inline_keyboard: [[{ text: "📩 OTP GROUP", url: GROUP_INVITE_LINK }], [{ text: `➡️ Next ${limit} Numbers`, callback_data: `assign_next_${last.country}` }], [{ text: "⬅️ Back", callback_data: "menu_country_fb" }]] };
    bot.editMessageText(buildAssignedMessageText(last.country, last.nums, last.statuses), { chat_id: chatId, message_id: messageId, reply_markup: actionMenu, parse_mode: "Markdown" }).then(() => { last.nums.forEach(num => { if (last.statuses[num].includes("Waiting")) { pendingRequests[num] = { chatId: chatId, messageId: messageId, country: last.country, batchNums: last.nums, allStatuses: last.statuses }; inUseNumbers[num] = true; } }); }).catch(()=>{});
    bot.answerCallbackQuery(query.id, { text: "✅ Restored successfully!" });
  }
    ...
    try {
      ...
      if (!token) {
        return bot.editMessageText(`❌ **Session Expired!**\n\nYour cookies are invalid or expired. Please Login from the Admin Panel to submit new values.`, { chat_id: chatId, message_id: messageId, reply_markup: { inline_keyboard: [[{ text: "⬅️ Back", callback_data: "admin_panel" }]] }, parse_mode: "Markdown" });
      }
      
      const resData = await iva.getNumbers(token);
      let grouped = {}; 
    ...
    if (!tempAdminData[chatId]) return bot.answerCallbackQuery(query.id, { text: "⚠️ Session expired! Fetch again.", show_alert: true });
    if (action === "addall") {
       let totalAdded = 0; tempAdminData[chatId].forEach(r => { if (!db.availableNumbers[r.name]) db.availableNumbers[r.name] = []; r.nums.forEach(num => { if (!db.availableNumbers[r.name].includes(num) && !inUseNumbers[num]) { db.availableNumbers[r.name].push(num); totalAdded++; } }); });
       saveDB(); bot.answerCallbackQuery(query.id, { text: `✅ Added all (${totalAdded})!` });
       ...
       const idx = parseInt(action), selected = tempAdminData[chatId][idx];
       if (db.availableNumbers[selected.name]) { delete db.availableNumbers[selected.name]; saveDB(); bot.answerCallbackQuery(query.id, { text: `❌ Removed ${selected.name}` }); }
       else { db.availableNumbers[selected.name] = []; let added = 0; selected.nums.forEach(num => { if (!inUseNumbers[num]) { db.availableNumbers[selected.name].push(num); added++; } }); saveDB(); sendStockAlert(selected.name, selected.nums.length); bot.answerCallbackQuery(query.id, { text: `✅ Added ${selected.name} (${added})` }); }
    }
    renderManageRangesMenu(chatId, messageId);
  }
  else if (data === "admin_check_db") {
    let text = "📊 **Stored Numbers:**\n\n", total = 0;
    for (const [country, nums] of Object.entries(db.availableNumbers)) { if (nums.length > 0) { text += `${getCountryInfo(country).flag} ${country}: ${nums.length}\n`; total += nums.length; } }
    bot.editMessageText(text + `\n**Total:** ${total}`, { chat_id: chatId, message_id: messageId, reply_markup: { inline_keyboard: [[{ text: "⬅️ Back", callback_data: "admin_panel" }]] }, parse_mode: "Markdown" }); bot.answerCallbackQuery(query.id);
  }
  else if (data === "admin_manage_admins") {
    if (!isSuperAdmin(chatId)) return;
    bot.editMessageText("👑 **Admin Management:**", { chat_id: chatId, message_id: messageId, reply_markup: { inline_keyboard: [[{ text: "➕ Add Admin", callback_data: "admin_add_admin" }, { text: "➖ Remove Admin", callback_data: "admin_remove_admin" }], [{ text: "⬅️ Back", callback_data: "admin_panel" }]] }, parse_mode: "Markdown" }); bot.answerCallbackQuery(query.id);
  }
    ...
    if (!isSuperAdmin(chatId)) return; if (db.adminUsernames.length === 0) return bot.answerCallbackQuery(query.id, { text: "📭 No admins found!", show_alert: true });
    ...
    bot.editMessageText("🗑️ **Select admin to remove:**", { chat_id: chatId, message_id: messageId, reply_markup: { inline_keyboard: btns }, parse_mode: "Markdown" }); bot.answerCallbackQuery(query.id);
  }
  else if (data.startsWith("deladmin_")) {
    if (!isSuperAdmin(chatId)) return; let unToRemove = data.replace("deladmin_", ""); db.adminUsernames = db.adminUsernames.filter(u => u !== unToRemove); saveDB();
    bot.answerCallbackQuery(query.id, { text: `✅ ${unToRemove} removed!`, show_alert: true });
    bot.editMessageText("👑 **Admin Management:**", { chat_id: chatId, message_id: messageId, reply_markup: { inline_keyboard: [[{ text: "➕ Add Admin", callback_data: "admin_add_admin" }, { text: "➖ Remove Admin", callback_data: "admin_remove_admin" }], [{ text: "⬅️ Back", callback_data: "admin_panel" }]] }, parse_mode: "Markdown" });
  }
...
function processFoundOTP(number, time, message, range) {
  ...
  const countryInfo = getCountryInfo(range || "UNKNOWN");
  const displayOtp = otpCode ? otpCode : "Not Found";
  ...
  const groupMsg = `✅ **New OTP Received** ✅\n\n` +
                   ...
                   `🔵 **Service:** FACEBOOK\n\n` +
                   `💌 **Full SMS:**\n❝ ${message} ❞`;
  bot.sendMessage(GROUP_CHAT_ID, groupMsg, { parse_mode: "Markdown" }).catch(()=>{});

  if (pendingRequests[number]) {
    const reqData = pendingRequests[number];
    reqData.allStatuses[number] = `✅ \`${otpCode || message}\``;
    
    if (db.lastAssigned[reqData.chatId] && db.lastAssigned[reqData.chatId].nums.includes(number)) {
      db.lastAssigned[reqData.chatId].statuses[number] = reqData.allStatuses[number];
      saveDB();
    }

    const updatedText = buildAssignedMessageText(reqData.country, reqData.batchNums, reqData.allStatuses);
    const limit = db.settings.maxNumbers || 4;
    const actionMenu = {
      inline_keyboard: [
        [{ text: "📩 OTP GROUP", url: GROUP_INVITE_LINK }],
        [{ text: `➡️ Next ${limit} Numbers`, callback_data: `assign_next_${reqData.country}` }],
        [{ text: "⬅️ Back", callback_data: "menu_country_fb" }]
      ]
    };

    bot.editMessageText(updatedText, { chat_id: reqData.chatId, message_id: reqData.messageId, reply_markup: actionMenu, parse_mode: "Markdown" }).catch(()=>{});
...
async function fastPendingOTPCheck() {
  ...
  const pNums = Object.keys(pendingRequests);
  ...
  try {
    ...
    if (token) {
      ...
      await iva.makeRequest("POST", "/portal/sms/received/getsms", parts, `multipart/form-data; boundary=${boundary}`, { "Referer": `${iva.BASE_URL}/portal/sms/received` });
      ...
      for (const range of rangesToCheck) {
        ...
        await iva.makeRequest("POST", "/portal/sms/received/getsms/number", b2, "application/x-www-form-urlencoded", { "Referer": `${iva.BASE_URL}/portal/sms/received` });

        const numbersInRange = pNums.filter(n => pendingRequests[n].country === range);

        await Promise.all(numbersInRange.map(async (number) => {
          const b3 = new URLSearchParams({ _token: token, start: today, end: today, Number: number, Range: range }).toString();
          const r3 = await iva.makeRequest("POST", "/portal/sms/received/getsms/number/sms", b3, "application/x-www-form-urlencoded", { "Referer": `${iva.BASE_URL}/portal/sms/received` }).catch(() => null);

          if (r3) {
            const msgs = iva.parseSMSMessages(r3.body, range, number, today);
            for (let i = 0; i < msgs.length; i++) {
              processFoundOTP(number, msgs[i][0], msgs[i][4], range);
            }
          }
        }));
      }
...
let isHeavyChecking = false;

async function checkAllOTP() {
  if (isHeavyChecking) return;
  isHeavyChecking = true;

  try {
    const token = cachedToken || await iva.fetchToken();
    if (token) {
      const resData = await iva.getSMS(token);
      if (resData.aaData && resData.aaData.length > 0) {
        resData.aaData.forEach(row => {
          processFoundOTP(row[2], row[0], row[4], row[1]);
        });
      }
    }
  } catch (err) {}
...
mongoose.connect(MONGODB_URI).then(async () => {
  ...
  if(db.cookies && db.cookies["XSRF-TOKEN"]) {
      ...
      console.log("🟢 Loaded cookies from MongoDB on startup.");
  }
