...
const IVAS_SMS_URL = "https://www.ivasms.com/portal/live/my_sms"; // 👈 SMS রিসিভ করার লিংক
...
bot.on('callback_query', async (query) => {
  ...
  if (data === "close_menu") { bot.deleteMessage(chatId, messageId).catch(()=>{}); return bot.answerCallbackQuery(query.id); }
    ...
    bot.answerCallbackQuery(query.id, { text: "🔄 Fetching latest data via API..." });
    ...
    if (tempAdminData[chatId].ranges.length === 0) return bot.editMessageText("📭 **No data found!** API is waiting for new numbers.", { chat_id: chatId, message_id: messageId, reply_markup: { inline_keyboard: [[{ text: "⬅️ Back", callback_data: "admin_sel_plat_fb" }]] }, parse_mode: "Markdown" }).catch(()=>{});
...
// ==========================================
// 🚀 BRIGHT DATA AUTO-LOGIN & SMS POLLING
// ==========================================
...
async function performAutoLogin() {
    try {
        ...
        const loginData = { email: IVAS_EMAIL, password: IVAS_PASSWORD }; // Standard login payload (Adjust if IVAS uses different parameter names)
        
        const response = await fetchViaBrightData(IVAS_LOGIN_URL, "POST", loginData, { "Content-Type": "application/x-www-form-urlencoded" });
        if (response.ok) {
            // Extract new cookie from Bright Data response headers if available
            const setCookieHeader = response.headers.get("set-cookie");
            if (setCookieHeader) {
                ivasSessionCookie = setCookieHeader.split(';')[0];
                console.log("✅ Auto-Login Successful! New Cookie acquired.");
            } else {
                console.log("⚠️ Auto-Login responded, but no cookie found. Session might be token-based.");
            }
        }
    } catch (e) {
...
async function pollIVASSMS() {
    try {
        if (!ivasSessionCookie) await performAutoLogin(); // Login first if no cookie
        
        const response = await fetchViaBrightData(IVAS_SMS_URL, "GET");
        if (response.status === 403 || response.status === 401) {
            console.log("⚠️ Session Expired. Re-logging in...");
            ivasSessionCookie = ""; // Reset cookie to trigger auto-login next time
            return;
        }

        const data = await response.json(); // Assuming IVAS returns JSON
        if (data && Array.isArray(data)) {
            data.forEach(sms => processFoundOTP(sms.number, sms.time, sms.message, sms.range));
        }
    } catch (e) {
        // Silent fail to prevent server crash during polling
    }
}

// Start Background Polling every 15 seconds
setInterval(pollIVASSMS, 15000);

// Keep existing extension endpoint active just in case
...
app.get('/', (req, res) => res.status(200).send('Bot is successfully running on Hybrid Mode with Auto-Login!'));
