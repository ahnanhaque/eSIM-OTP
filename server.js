...
const TG_API_ID   = 34141457;
const TG_API_HASH = "840906716a5ac6e27588fa1f16469516";
...
const dbSchema = new mongoose.Schema({
    ...
    userbotSession:     String, // 🟢 NEW: Userbot Session
    targetBotUsername:  String  // 🟢 NEW: Partner Bot Username
}, { strict: false });
...
// 🟢 Userbot Live Variables
let activeLoginClient      = null;
let activePhoneCodeHash    = "";
let liveUserbotInstance    = null;
...
// ============================================================
// #  LIVE USERBOT LOADER & INTEGRATION
// ============================================================

async function initLiveUserbot() {
    if (!db.userbotSession) return console.log("⚠️ No active userbot session string in database.");
    try {
        const session = new StringSession(db.userbotSession);
        liveUserbotInstance = new TelegramClient(session, TG_API_ID, TG_API_HASH, { connectionRetries: 5 });
        await liveUserbotInstance.connect();
        console.log("✅ [Userbot] Dummy Telegram Account successfully connected on Render server!");
        
        liveUserbotInstance.addEventHandler(async (event) => {
            const message = event.message;
            if (!message || !db.targetBotUsername) return;
            
            try {
                const entity = await liveUserbotInstance.getEntity(message.peerId);
                if (entity && entity.username && entity.username.toLowerCase() === db.targetBotUsername.toLowerCase().replace("@", "")) {
                    console.log(`[Userbot Scraped Message from ${entity.username}]:`, message.message);
                    // 💡 NOTE: OTP extract logic will be mapped here
                }
            } catch(e) {}
        });
    } catch (e) {
        console.log("❌ [Userbot] Connection failed:", e.message);
    }
}
...
const countryPrefixes = {
    "1": "USA", "7": "RUSSIA", "880": "BANGLADESH", "228": "TOGO" // Truncated for space, assume all exist
};

const countryData = {
    "TOGO": { flag: "🇹🇬" }, "BANGLADESH": { flag: "🇧🇩" }, "USA": { flag: "🇺🇸" }
};
...
function getAdminMenu(chatId) {
    let menu = [
        ...
        [{ text: "🤖 Userbot Control",   callback_data: "admin_userbot_dashboard" }] // 🟢 NEW
    ];
...
// 🟢 NEW: Userbot Menu UI
function getUserbotDashboardUI() {
    let statusIcon = liveUserbotInstance && liveUserbotInstance.connected ? "🟢 Connected" : "🔴 Disconnected";
    let target = db.targetBotUsername || "Not Configured ❌";
    return {
        text: `🤖 **Userbot (Aggregator Mode) Dashboard:**\n\n` +
              `📡 **Status:** ${statusIcon}\n` +
              `🤝 **Partner Bot:** \`${target}\`\n\n` +
              `Configure your backend dummy account directly via Render & GitHub without running scripts locally.`,
        markup: {
            inline_keyboard: [
                [{ text: "🔑 Link Dummy Account", callback_data: "admin_ub_link_start" }],
                [{ text: "🤝 Set Partner Bot Username", callback_data: "admin_ub_target_username" }],
                [{ text: "🔌 Disconnect Userbot", callback_data: "admin_ub_disconnect" }],
                [{ text: "⬅️ Back", callback_data: "admin_panel" }]
            ]
        }
    };
...
bot.on("message", async (msg) => {
    ...
    // 🟢 1. Userbot Live Login Interactions
    if (userStates[chatId] === "WAITING_FOR_UB_PHONE" && isAdmin(chatId, username)) {
        let phone = text.replace(/\s+/g, "");
        bot.sendMessage(chatId, `⏳ Connecting to Telegram servers & requesting OTP for \`${phone}\`...`, { parse_mode: "Markdown" }).catch(() => {});
        
        try {
            activeLoginClient = new TelegramClient(new StringSession(""), TG_API_ID, TG_API_HASH, { connectionRetries: 5 });
            await activeLoginClient.connect();
            
            const res = await activeLoginClient.sendCode({ apiId: TG_API_ID, apiHash: TG_API_HASH }, phone);
            activePhoneCodeHash = res.phoneCodeHash;
            tempAdminData[chatId] = { userbotPhone: phone };
            
            userStates[chatId] = "WAITING_FOR_UB_OTP";
            bot.sendMessage(chatId, `📩 **OTP Sent!** Please check your official Telegram account and send the OTP code here.\n\n_Example format: 12345_`, { parse_mode: "Markdown" }).catch(() => {});
        } catch (e) {
            bot.sendMessage(chatId, `❌ **GramJS Error:** ${e.message}`).catch(() => {});
            delete userStates[chatId];
        }
        return;
    }

    if (userStates[chatId] === "WAITING_FOR_UB_OTP" && isAdmin(chatId, username)) {
        let otp = text.trim();
        let phone = tempAdminData[chatId]?.userbotPhone;
        bot.sendMessage(chatId, `⏳ Completing authentication and saving session string...`).catch(() => {});
        
        try {
            await activeLoginClient.signIn({ phoneNumber: phone, phoneCodeHash: activePhoneCodeHash, phoneCode: otp });
            const sessionStr = activeLoginClient.session.save();
            
            db.userbotSession = sessionStr;
            saveDB();
            
            bot.sendMessage(chatId, `✅ **Success!** Your Dummy Account has been successfully linked via Render. Session token is stored in MongoDB!`).catch(() => {});
            delete userStates[chatId];
            delete tempAdminData[chatId];
            
            initLiveUserbot();
        } catch(e) {
            bot.sendMessage(chatId, `❌ **Sign-In Failed:** ${e.message}`).catch(() => {});
            delete userStates[chatId];
        }
        return;
    }

    if (userStates[chatId] === "WAITING_FOR_UB_TARGET" && isAdmin(chatId, username)) {
        let target = text.trim().replace("@", "");
        db.targetBotUsername = target;
        saveDB();
        bot.sendMessage(chatId, `✅ Partner bot configuration updated to: \`@${target}\``, { parse_mode: "Markdown" }).catch(() => {});
        delete userStates[chatId];
        
        let ui = getUserbotDashboardUI();
        bot.sendMessage(chatId, ui.text, { reply_markup: ui.markup, parse_mode: "Markdown" }).catch(() => {});
        return;
    }
    ...
    if (text === "☎️ Get Number") {
        ...
        if (panel === "stex") {
            ...
            saveDB();
            bot.sendMessage(chatId,
                `✅ Successfully added Zenex Range **${range}** for **${platform.toUpperCase()}**.\n🌍 Country: **${country}**\n📝 Method: **${method}**`,
                { parse_mode: "Markdown" }
            ).catch(() => {});
        }
...
bot.on("callback_query", async (query) => {
    ...
    // 🟢 2. Userbot UI Callbacks
    else if (data === "admin_userbot_dashboard") {
        let ui = getUserbotDashboardUI();
        bot.editMessageText(ui.text, { chat_id: chatId, message_id: messageId, reply_markup: ui.markup, parse_mode: "Markdown" }).catch(() => {});
        bot.answerCallbackQuery(query.id);
    }

    else if (data === "admin_ub_link_start") {
        userStates[chatId] = "WAITING_FOR_UB_PHONE";
        bot.sendMessage(chatId, `📱 **Linking Userbot Account:**\nPlease send your Dummy account's phone number with full country code.\n\n_Example: +88017XXXXXXXX_`, { parse_mode: "Markdown" }).catch(() => {});
        bot.answerCallbackQuery(query.id);
    }

    else if (data === "admin_ub_target_username") {
        userStates[chatId] = "WAITING_FOR_UB_TARGET";
        bot.sendMessage(chatId, `🤝 **Collaboration Settings:**\nPlease send the username of the Telegram Bot you want to scrape numbers/OTPs from.\n\n_Example: eSIM_OTP_Bot_`, { parse_mode: "Markdown" }).catch(() => {});
        bot.answerCallbackQuery(query.id);
    }

    else if (data === "admin_ub_disconnect") {
        db.userbotSession = "";
        saveDB();
        if (liveUserbotInstance) { 
            try { await liveUserbotInstance.disconnect(); } catch(e){} 
            liveUserbotInstance = null; 
        }
        bot.answerCallbackQuery(query.id, { text: "🔌 Userbot session cleared successfully!", show_alert: true });
        let ui = getUserbotDashboardUI();
        bot.editMessageText(ui.text, { chat_id: chatId, message_id: messageId, reply_markup: ui.markup, parse_mode: "Markdown" }).catch(() => {});
    }
    ...
    else if (data === "zenex_login") {
        let btns = [];
        if (db.savedZenexAccounts && db.savedZenexAccounts.length > 0) {
            db.savedZenexAccounts.forEach((acc, idx) => {
                let activeMark = (db.zenexCreds && db.zenexCreds.email === acc.email) ? "✅ " : "👤 ";
                btns.push([{ text: `${activeMark}${acc.email}`, callback_data: `zenex_qlogin_${idx}` }]);
            });
            btns.push([{ text: "Remove Account", callback_data: "zenex_remove_menu" }]);
        }
        btns.push([{ text: "Add Account", callback_data: "zenex_manual_login" }]);
        btns.push([{ text: "⬅️ Back", callback_data: "admin_manage_panel" }]);
        bot.editMessageText("🔑 **Zenex SMS Login:**\nChoose an account to login or add a new one:",
            { chat_id: chatId, message_id: messageId, parse_mode: "Markdown", reply_markup: { inline_keyboard: btns } }
        ).catch(() => {});
        bot.answerCallbackQuery(query.id);
    }

    else if (data === "zenex_remove_menu") {
        let btns = [];
        (db.savedZenexAccounts || []).forEach((acc, idx) => {
            btns.push([{ text: `❌ ${acc.email}`, callback_data: `zenex_delacc_${idx}` }]);
        });
        btns.push([{ text: "⬅️ Back", callback_data: "zenex_login" }]);
    ...
    else if (data.startsWith("zenex_delacc_")) {
        const idx = parseInt(data.replace("zenex_delacc_", ""));
        const acc = db.savedZenexAccounts[idx];
        if (acc) {
            if (db.zenexCreds && db.zenexCreds.email === acc.email) { db.zenexCreds = null; db.zenexCookies = ""; }
            db.savedZenexAccounts.splice(idx, 1);
            saveDB();
        ...
        bot.editMessageText("🔑 **Zenex SMS Login:**", 
            { chat_id: chatId, message_id: messageId, parse_mode: "Markdown", reply_markup: { inline_keyboard: [[{ text: "⬅️ Back", callback_data: "zenex_login" }]] } }
        ).catch(() => {});
    }

    else if (data === "zenex_manual_login") {
        userStates[chatId] = "WAITING_FOR_ZENEX_CREDS";
        bot.sendMessage(chatId, "📧 **Send Zenex credentials format:**\n`email|password`", { parse_mode: "Markdown" }).catch(() => {});
        bot.answerCallbackQuery(query.id);
    }

    else if (data.startsWith("zenex_qlogin_")) {
        const idx = parseInt(data.replace("zenex_qlogin_", ""));
        const acc = db.savedZenexAccounts[idx];
        if (acc) {
            bot.sendMessage(chatId, "⏳ Logging into Zenex SMS...").catch(() => {});
            zenex.login(acc.email, acc.password).then(cookieStr => {
                db.zenexCookies = cookieStr; db.zenexCreds = acc; saveDB();
                bot.sendMessage(chatId, "✅ Zenex Login Successful! Session is saved.").catch(() => {});
            }).catch(e => bot.sendMessage(chatId, "❌ Failed: " + e.message).catch(() => {}));
        }
        bot.answerCallbackQuery(query.id);
    }
        ...
        bot.sendMessage(chatId,
            `🔢 **Enter Zenex Range for ${platform.toUpperCase()}:**\nJust type the range, the bot will automatically detect the country.\nExample: \`23276XXX\``,
            { parse_mode: "Markdown" }
        ).catch(() => {});
        bot.answerCallbackQuery(query.id);
    }
...
mongoose.connect(MONGODB_URI).then(async () => {
    ...
    isDbLoaded = true;
    app.listen(PORT, () => console.log(`🚀 Webhook & Userbot Mode running on port ${PORT}`));

    setTimeout(autoLoginPanels, 10000);
    setTimeout(initLiveUserbot, 5000); // 🟢 Auto-start live userbot listener
